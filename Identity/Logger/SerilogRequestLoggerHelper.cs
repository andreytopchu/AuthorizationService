...
public static class SerilogRequestLoggerHelper
{
    ...
    private static readonly MapperConfiguration MapperConfiguration = new(cfg =>
    {
        cfg.CreateMap<HttpRequest, HttpRequestInfo>()
            .ForMember(info => info.Action,
                expression => expression.MapFrom(request => request.RouteValues["action"].ToString()))
            .ForMember(info => info.Controller,
                expression => expression.MapFrom(request => request.RouteValues["controller"].ToString()))
            .ForMember(info => info.QueryString,
                expression => expression.MapFrom(request => request.QueryString.ToString()))
    ...
    public static void SetupSerilogEnrichAction(RequestLoggingOptions options)
    {
        ...
        options.EnrichDiagnosticContext = (diagnosticsContext, httpContext) =>
        {
            ...
            foreach (var keyValuePair in requestInfo.AsArray())
            {
